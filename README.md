...
- OpenAI Whisper API（語音轉文字，唯一會呼叫外部服務的功能）
...
| 變數 | 用途 |
| --- | --- |
| `OPENAI_API_KEY` | 語音轉文字（Whisper）——這是整個 App 唯一需要的 AI 相關金鑰 |
...
1. 到 https://platform.openai.com/ 建立 `OPENAI_API_KEY`，用於 `/api/stt`（Whisper），這是唯一會把使用者內容送到外部服務的功能，且只有在使用者主動按下「轉換成文字」時才會送出。
